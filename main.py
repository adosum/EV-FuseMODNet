...
from model.models import evMODNet
...
def train(train_loader, net, optimizer, epoch, train_writer):
    ...
    for batch in tqdm(train_loader):
        ...
        pred, _, _ = net(former_gray, latter_gray, eventdata)
...
def validate(test_loader, net, epoch):
    ...
    for i in range(len(test_loader)):
        ...
        pred, flow_ev, flow_rgb = net(former_gray, latter_gray, eventdata)
        ...
        if args.render and epoch < 0:
            ...
            flow_ev = flow_viz_np(flow_ev.cpu().numpy()[0, 0, :, :], flow_ev.cpu().numpy()[0, 1, :, :])
            cv2.imshow('evflow', flow_ev)
